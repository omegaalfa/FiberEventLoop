...
trait TimerManagerTrait
{
    ...
    /**
     * Executa um ciclo de trabalho seguindo a mesma prioridade do loop principal.
     *
     * @return bool True quando processou pelo menos uma unidade de trabalho.
     */
    protected function processPrioritizedWorkCycle(): bool
    {
        $hadWork = false;
    ...
    /**
     * Tick cooperativo do loop durante sleep()
     *
     * Executa um ciclo completo do event loop (streams, fibers, timers e deferred)
     * para evitar starvation enquanto uma Fiber está aguardando.
     *
     * @return void
     */
    protected function tick(): void
    {
        $hadWork = $this->processPrioritizedWorkCycle();
        ...
        if ($sleep === null) {
            usleep(200); // Sleep curto para evitar busy-loop sem adicionar latência perceptível.
            return;
        }

        if ($sleep > 0.0) {
            $sleepUs = (int) min($sleep * 1_000_000, 1_000);
            if ($sleepUs > 0) {
                usleep($sleepUs);
            }
        }
